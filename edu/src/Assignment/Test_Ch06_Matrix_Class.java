...
class Matrix {
	...
	Matrix multiplyMatrix(Matrix e) { //D = B.multiplyMatrix(E); (3x4) x (4x5)
		int sum = 0;
		Matrix m = new Matrix(this.rows, e.cols);
		for (int i=0; i<this.rows; i++) {
			for (int j=0; j<e.cols; j++) {
				for (int k=0; k<e.rows; k++) {
					sum = sum + this.data[i*this.cols + k] * e.data[j+k*e.cols];
				}
				m.data[i*e.cols + j]=sum;
				sum = 0;
			}
		}
		return m;	
	...
	Matrix transposeMatrix() {
		...
		for (int i=0; i<this.cols; i++) {
			for (int j=0; j<this.rows; j++){
				m.data[i*this.rows + j] = this.data[j*this.cols + i]; 
			}
		}
