...
class Tree5 {
	...
	//stack을 사용한 inorder 출력
	{
		...
		while (true) {
			...
			if (!s.isEmpty()) {
				try {
					CurrentNode = s.pop();
				} catch (datastructure.chap09.ObjectStack5.EmptyGenericStackException e) {
	...
	boolean insert(int x) {// binary search tree를 만드는 입력 : left subtree < 노드 x < right subtree
		...
		TreeNode5 p = root; // 먼저 가고
		TreeNode5 q = null; // q가 따라가고 
		
		TreeNode5 newTree = new TreeNode5();
		newTree.data = x;
			...
			while(p!=null) {
				...
				//같으면 왼쪽에 할당
					...
					if(p.LeftChild == null) {
						...
						newTree.LeftChild = p.LeftChild;
						p.LeftChild = newTree.LeftChild;
						return false;
					}
				}
			}
			// p가 null : 노드 끝까지 왔다.
			// => q보다 작으면 왼쪽, 크거나 같으면 오른쪽
			if(x < q.data) 
				q.LeftChild = newTree;
			else 
...
public class 정수이진트리 {
	...
	public static void main(String[] args) {
		...
		Tree5 t = new Tree5();
		Menu menu; // 메뉴
		int count = 20; //난수 생성 개수
		int num;
		boolean result;
		do {
			switch (menu = SelectMenu()) {
			case Add: // 
				System.out.println("삽입삽입");
