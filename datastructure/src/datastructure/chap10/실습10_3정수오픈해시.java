...
class OpenHash2 {
	...
	public int add(int data) {
		int indx = hashValue(data);
		
		Bucket current = table[indx]; 
		
		
		// Table이 Full일 때 (indx 부터 끝까지)
		int isFull = 0;
		for(int i = indx; i<size; i++) {
			if(current.stat != Status.EMPTY)
				isFull++;
		} if(isFull == size) return 2;
		
		// 비어있으면 넣어라
		if (current.stat == Status.EMPTY) {
			current.data = data;
			current.stat = Status.OCCUPIED;
			return data;
		}
