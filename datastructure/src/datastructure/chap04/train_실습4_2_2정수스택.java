...
//--- 생성자(constructor) ---//
	public IntStack4(int maxlen) {
		...
		capacity = maxlen;
		try {
			this.stk = new ArrayList<Integer>(maxlen);
...
//--- 스택에 x를 푸시 ---//
	public void push(int x) throws OverflowIntStackException {
		...
		if (isFull()) {
			throw new OverflowIntStackException("push: Stack이 가득 참");// 스택이 가득 찼다고 하면서 예외를 출력
		}
		this.stk.add(x);
		ptr ++;
	}
...
//--- 스택에서 데이터를 팝(정상에 있는 데이터를 꺼냄) ---//
	public int pop() throws EmptyIntStackException {
		if (isEmpty()) {
			// 스택이 빔
			throw new EmptyIntStackException("pop: Stack이 비었음");		
		}
		int temp = stk.get(--ptr);
		stk.remove(ptr);
		return temp;
	}
...
//		if (ptr<= 0) 대신에 사용:	
		if (isEmpty()) // 스택이 빔
			throw new EmptyIntStackException("peek: Stack이 비었음");
...
//--- 스택을 비움 ---//
	public void clear() throws EmptyIntStackException {
		 ...
		 */
		if (isEmpty()) // 스택이 빔
			throw new EmptyIntStackException("clear: Stack이 비었음");
...
//--- 스택 안의 모든 데이터를 바닥 → 정상 순서로 표시 ---//
	public void dump() throws EmptyIntStackException{
		if (isEmpty()) {
			throw new EmptyIntStackException("dump: 데이터가 비었습니다.");
		}
		for(int item : stk) {
...
public class train_실습4_2_2정수스택 {

	public static void main(String[] args) {
		Scanner stdIn = new Scanner(System.in);
		IntStack4 s = new IntStack4(4); // 크기가 4인 정수스택 (최대 4 개를 푸시할 수 있는 스택)
