...
//		int num = 0; // p = null로 대체해서 사용
	...
	public void Add(int element) // 임의 값을 삽입할 때 리스트가 오름차순으로 정렬이 되도록 한다
	{
		...
		if (first == null) // insert into empty list
		{
			...
			if(first.data > element) {// element가 first보다 작을 때 : first가 됨.
				newNode.link = p;
				first = newNode;
				return;
			}
...
//				if (element >= q.data && p == null) {
//					q.link = newNode;
//				}	
				if (element >= q.data && element <= p.data) {
					q.link = newNode;
					newNode.link = p;
					break;
				}
...
public class 실습9_1정수연결리스트 {
	enum Menu { // enum 클래스는 생성자를 갖지 않는다.
		...
		Menu(String string) { 
			...
			// Enum 클래스의 생성자는 개발자가 직접 호출하여 인스턴스를 생성하는 것이 아니라, 
			// enum 상수가 정의될 때 내부적으로 호출됨. 
			// 따라서 이 생성자는 일종의 초기화 메서드로 간주할 수 있겠다. 
			// Enum 상수가 정의될 때 생성자가 호출되어 각 상수의 필드를 초기화하고, 이후에는 해당 상수 사용가능.
			// 생성자는 enum 상수의 정의 시점에서 한 번만 호출, 이후에는 해당 상수를 사용할 때마다 생성자가 다시 호출 X
	...
	// --- 메뉴 선택 ---//
	static Menu SelectMenu() { // return 값이 Menu인 메서드
		Scanner sc = new Scanner(System.in);
		int key;
		do {
			for (Menu m : Menu.values()) { //메뉴 출력 보여주는 코드
				System.out.printf("(%d) %s  ", m.ordinal(), m.getMessage());
				//n%3은 3으로 나누어 나머지를 계산한다 
				if ((m.ordinal() % 3) == 2 && m.ordinal() != Menu.Exit.ordinal())//(2)출력 이후 한줄 개행
					System.out.println();
			}
			System.out.print(" : ");
